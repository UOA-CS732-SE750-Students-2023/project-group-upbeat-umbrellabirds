                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', overflowY: 'scroll', height: window.innerHeight - 90, width: '100%', overflowX: 'hidden' }}>
                    {
                        roomlist.map((item, index) => {
                            return (
                                <div style={{ width: '18%', backgroundColor: 'rgba(17, 83, 165, 0.5)', height: '300px', margin: '2.5%', borderRadius: '10px', color: "white", display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', padding: '10px' }}>
                                    {/* Display room number */}
                                    ...
                                    <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        {/* Render start button for the leader and join button for others */}
                                        {isLeader === index ? (
                                            <Button type="primary" className="button_sb" style={{ height: "43px", backgroundColor: "#00ffff" }} onClick={onStart}>
                                                start
                                            </Button>
                                        ) : (
                                            <Button type="primary" className="button_sb" style={{ height: "43px", backgroundColor: "#00ffff" }}
                                                onClick={onJoin}>
                                                join
                                            </Button>
                                        )}
